...
public final class StyleUtils {
  ...
  public static void styleRun(XWPFRun destRun, XWPFRun srcRun) {
    ...
    if (Boolean.TRUE.equals(srcRun.isBold())) {
    ...
    if (Boolean.TRUE.equals(srcRun.isItalic())) {
  ...
  public static void styleTableParagraph(XWPFParagraph par, TableStyle style) {
    if (null != par && null != style) {
      ...
      if (null != style.getIndentation()) {
        CTInd ctInd = CTPpr.isSetInd() ? CTPpr.getInd() : CTPpr.addNewInd();
        ctInd.setFirstLine(new BigInteger("0"));
        ctInd.setFirstLineChars(style.getIndentation().multiply(new BigInteger("100")));
      }
      if (null != style.getSpacing()) {
        CTSpacing pSpacing = CTPpr.isSetSpacing() ? CTPpr.getSpacing() : CTPpr.addNewSpacing();
        pSpacing.setLine(style.getSpacing().multiply(new BigInteger("20")));
        pSpacing.setLineRule(STLineSpacingRule.EXACT);
      }
      if (null != style.getHorizonalAlignment()) {
        par.setAlignment(style.getHorizonalAlignment());
      }
    }
  }
